...
class LectureVideo(Model,
    ...
    def image_path(self, instance, filename):
        upload_to = 'Lecture/Video/ThumbnailImage/'
        time = strftime("%Y%m%dT%H%M%S", gmtime())
        ext = filename.split('.')[-1]
        filename = f'{time}.{ext}'
        return os.path.join(upload_to, filename)
    ...
    video = models.FileField(_('비디오'), upload_to=video_path)
    video_url = models.URLField(_('비디오 URL'), null=True, blank=True)
    thumbnail_image = models.ImageField(_('썸네일 이미지'), null=True, blank=True, upload_to=image_path)
...
class LectureImage(Model):
    ...
    def thumbnail_image_path(self, instance, filename):
        upload_to = 'Lecture/DetailImage/ThumbnailImage/'
        time = strftime("%Y%m%dT%H%M%S", gmtime())
        ext = filename.split('.')[-1]
        filename = f'{time}.{ext}'
        return os.path.join(upload_to, filename)
    ...
    thumbnail_image = models.ImageField(_('썸네일 이미지'), null=True, blank=True, upload_to=thumbnail_image_path)
    thumbnail_image_url = models.URLField(_('썸네일 이미지 URL'), null=True, blank=True)
